...
if selected == "Model":
    ...
    if st.button("Decision Tree"):
        ...
        with col2:
            ...
            st.write(pd.DataFrame(conf_matrix, index=["Actual Positive", "Actual Negative"], 
    ...
    if st.button("Random Forest"):
        ...
        with col1:
            st.markdown("<h3 style='text-align: center; color:White'>Accuracy</h3>", unsafe_allow_html=True)
            st.markdown(f"<div style='text-align: center; font-size: 24px; font-weight: bold;color:White'>{accuracy:.2f}</div>", unsafe_allow_html=True)

        with col2:
            st.markdown("<h3 style='text-align: center;color:White'>Confusion Matrix</h3>", unsafe_allow_html=True)
            st.write(pd.DataFrame(conf_matrix, index=["Actual Positive", "Actual Negative"], 
                                           columns=["Predicted Positive", "Predicted Negative"]))
